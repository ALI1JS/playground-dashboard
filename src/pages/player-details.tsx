...
const PlayerDetails: React.FC = () => {
    ...
    useEffect(() => {
        const fetchPlayerDetails = async () => {
            try {
                const res = await axios.get(`http://abdoo120-001-site1.ctempurl.com/api/Player/${id}`);
                setPlayer(res.data);
    ...
    const fetchStadiums = async (stadiumIds: string[], ownerStadiumIds: string[]) => {
        try {
            const stadiumRequests = stadiumIds.map(async (stadiumId) => {
                const res = await axios.get(`http://abdoo120-001-site1.ctempurl.com/api/Stadium/GetByStadiumId/${stadiumId}`);
                return res.data;
            });

            const ownerStadiumRequests = ownerStadiumIds.map(async (ownerStadiumId) => {
                const res = await axios.get(`http://abdoo120-001-site1.ctempurl.com/api/Owner/${ownerStadiumId}`);
                return res.data;
            });
                ...
                <div className="flex flex-col gap-20 w-[100%] p-10 bg-slate-100 absolute top-[150px]">
                    ...
                    <div className="flex flex-col gap-5 p-5 bg-white rounded">
                        ...
                        {player.proofOfIdentityUrl && (
                            <div className="mt-5">
                                <a href={`http://abdoo120-001-site1.ctempurl.com/${player.proofOfIdentityUrl}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">
