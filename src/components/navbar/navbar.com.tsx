  ...
  return (
    <div className={`bg-slate-100 xl:bg-white shadow-xl absolute top-[150px] xl:top-0 -translate-x-96 xl:translate-x-0 ${props.isHidden ? '-translate-x-96' : 'translate-x-0'} z-10 transition-transform min-h-[100%] w-[300px] flex flex-col gap-5`}>
      ...
      <div className='flex gap-2 flex-col bg-slate-100 p-5'>
        ...
        <Link className='flex gap-3 font-bold text-md' to="/admin/view"> <img className='w-5 h-5' src={ownersIcon} alt="catogery-icon" />Admin List</Link>
        <Link className='flex gap-3 font-bold text-md' to="/admin/create"> <img className='w-5 h-5' src={ownersIcon} alt="catogery-icon" />Create admin</Link>
      </div>

      <div className='flex gap-2 flex-col bg-slate-100 p-5'>
        <h2 className='font-bold text-2xl'>Users</h2>
        <Link className='flex gap-3 font-bold text-md' to="/user/owner/create"> <img className='w-5 h-5' src={ownersIcon} alt="catogery-icon" />Create Owner</Link>
        <Link className='flex gap-3 font-bold text-md' to="/user/player/create"> <img className='w-5 h-5' src={ownersIcon} alt="catogery-icon" />Create player</Link>
      </div>

      <div>
        <div className='flex gap-2 flex-col bg-slate-100 p-5'>
          <h2 className='font-bold text-2xl'>Chat</h2>
          <Link to="/chat" className="p-2 rounded-full"
          >
            <div className='flex gap-1 items-center justify-start'>
              <FaComments className='text-green-500 font-bold text-2xl'/>
        ...
        <div className='flex gap-2 flex-col bg-slate-100 p-5'>
          ...
          <Link className='flex gap-3 font-bold text-md' to="/report">
            <FaFileAlt style={{ color: 'green' }} className='w-5 h-5' /> {/* Report icon in green */}
            Report
          </Link>
        </div>
      </div>
