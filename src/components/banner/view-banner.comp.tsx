...
const ViewBanner: React.FC = () => {
    ...
    const deleteBanner = async (id: string, url: string) => {
        try {
            const deleteImageResponse = await axios.delete(`${import.meta.env.VITE_BASE_URL}/api/Banner/DeleteBannerImage`, {
                data: { bannerImageUrl: url }
            });


            if (deleteImageResponse.status === 200) {
                const deleteBannerResponse = await axios.delete(`${import.meta.env.VITE_BASE_URL}/api/Banner/${id}`);
                                ...
                                <div key={banner.bannerId} className="relative group">
                                    ...
                                    <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button
                                            ...
                                            onClick={() => deleteBanner(banner.bannerId, banner.bannerImageUrl)}
                                        >
